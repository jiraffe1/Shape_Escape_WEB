...
function preload() {
  ...
  three = loadImage("Axy1-removebg-preview.png");
  four = loadImage("Axy2-removebg-preview.png");
  one = loadImage("Knify1-removebg-preview.png");
  two = loadImage("Knify2-removebg-preview.png");
  playerIdle = loadImage("Player_Idle-removebg-preview.png");
  playerRun1 = loadImage("Player_Run1-removebg-preview.png");
  lev = loadStrings("level.txt");
}
...
function setup() {
 ...
 // }
  ...
  map = processing2jsNewNDimArray([w, h]);
    ...
    for (var i = -1; i < 2; i++) {
      for (var j = -1; j < 2; j++) {
        if (i + j != 0) {
          ...
          if (gmap(nx, ny) == 0 && !this.isInLast(nx, ny, this.searchDepth)) {
            if (ndist < bestDist) {
              bestPosX = nx;
              bestPosY = ny;
              bestDist = ndist;
